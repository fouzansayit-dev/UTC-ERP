...
export default function VideoImportantLink() {
  const [form, setForm] = useState({ course: 'ALL', branchName: '', batch: '', date: today(), category: '', linkType: 'YouTube', title: '', link: '', order: '' });
  ...
  const handleSubmit = () => {
    ...
    setForm({ course: 'ALL', branchName: '', batch: '', date: today(), category: '', linkType: 'YouTube', title: '', link: '', order: '' });
  };
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Video &amp; Important Link</div>
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '24px 28px', boxShadow: '0 2px 8px rgba(67,97,238,0.06)' }}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          <div className="form-field"><label className="form-label">Link Type</label><select className="form-input" value={form.linkType} onChange={e => set('linkType', e.target.value)}><option>YouTube</option><option>External URL</option></select></div>
          <div className="form-field" style={{ gridColumn: '1 / -1' }}><label className="form-label">Title</label><textarea className="form-input form-textarea" rows={2} value={form.title} onChange={e => set('title', e.target.value)} /></div>
          <div className="form-field" style={{ gridColumn: '1 / -1' }}><label className="form-label">Link</label><textarea className="form-input form-textarea" rows={2} value={form.link} onChange={e => set('link', e.target.value)} /></div>
      ...
      <div style={{ marginTop: 28 }}>
        <div style={{ background: '#1e293b', color: '#fff', padding: '14px 20px', borderRadius: '10px 10px 0 0', fontWeight: 700, fontSize: 15 }}>Video &amp; Important Link List</div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0' }}>
          <div style={{ display: 'flex', gap: 8 }}>{['Copy','CSV','Print'].map(b=><button key={b} className="tbl-btn view" style={{ background:'#f1f5f9',color:'#374151' }} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'VideoImportantLink'); else handlePrint('Video Important Link'); }}>{b}</button>)}</div>
        ...
        <div className="table-wrap" style={{ borderRadius: '0 0 10px 10px' }}>
          <table className="hr-table">
            <thead><tr>{['S.No','Edit','Delete','Course','Branch','Batch','Date','Title','Category','Order','Link Type','Link'].map(h=><th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={12} className="empty-table-msg">No data available in table</td></tr>
                : filtered.map((r,i)=><tr key={r.id}><td>{i+1}</td><td><button className="tbl-btn edit" onClick={()=>handleEdit(r)}>Edit</button></td><td><button className="tbl-btn del" onClick={()=>handleDelete(r.id)}>🗑</button></td><td>{r.course}</td><td>{r.branchName}</td><td>{r.batch}</td><td>{r.date}</td><td>{r.title}</td><td>{r.category}</td><td>{r.order}</td><td>{r.linkType}</td><td style={{maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.link}</td></tr>)}
            </tbody>
          </table>
        </div>
