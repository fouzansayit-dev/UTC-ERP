...
export default function Syllabus() {
  const [form, setForm] = useState({ course: '', branchName: '', batch: '', date: today(), subject: 'NA', currentYearSem: 'NA', syllabus: '', upload: null });
  ...
  const handleSubmit = () => {
    ...
    setForm({ course: '', branchName: '', batch: '', date: today(), subject: 'NA', currentYearSem: 'NA', syllabus: '', upload: null });
  };
  const handleEdit = row => { setEditId(row.id); setForm({ ...row }); };
  const handleDelete = id => setRows(p => p.filter(r => r.id !== id));
  const filtered = rows.filter(r => (r.syllabus || '').toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="hr-form">
      <div className="section-title">Syllabus</div>
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '24px 28px', boxShadow: '0 2px 8px rgba(67,97,238,0.06)' }}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          <div className="form-field"><label className="form-label">Current Year/Semester</label><select className="form-input" value={form.currentYearSem} onChange={e=>set('currentYearSem',e.target.value)}><option>NA</option><option>Year 1</option><option>Year 2</option><option>Year 3</option></select></div>
          <div className="form-field" style={{gridColumn:'1 / -1'}}><label className="form-label">Syllabus</label><textarea className="form-input form-textarea" rows={3} value={form.syllabus} onChange={e=>set('syllabus',e.target.value)} /></div>
          <div className="form-field"><label className="form-label">Upload</label><input className="form-input" type="file" onChange={e=>set('upload',e.target.files[0]?.name||null)} /></div>
        </div>
      ...
      <div style={{marginTop:28}}>
        <div style={{background:'#1e293b',color:'#fff',padding:'14px 20px',borderRadius:'10px 10px 0 0',fontWeight:700,fontSize:15}}>Syllabus List</div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 0'}}>
          <div style={{display:'flex',gap:8}}>{['Copy','CSV','Print'].map(b=><button key={b} className="tbl-btn view" style={{background:'#f1f5f9',color:'#374151'}} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'Syllabus'); else handlePrint('Syllabus'); }}>{b}</button>)}</div>
        ...
        <div className="table-wrap" style={{borderRadius:'0 0 10px 10px'}}>
          <table className="hr-table">
            <thead><tr>{['S.No','Edit','Delete','Course','Branch Name','Batch','Current Year/Semester','Date','Syllabus','Upload'].map(h=><th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.length===0?<tr><td colSpan={10} className="empty-table-msg">No data available in table</td></tr>
                :filtered.map((r,i)=><tr key={r.id}><td>{i+1}</td><td><button className="tbl-btn edit" onClick={()=>handleEdit(r)}>Edit</button></td><td><button className="tbl-btn del" onClick={()=>handleDelete(r.id)}>🗑</button></td><td>{r.course}</td><td>{r.branchName}</td><td>{r.batch}</td><td>{r.currentYearSem}</td><td>{r.date}</td><td>{r.syllabus}</td><td>{r.upload||'—'}</td></tr>)}
            </tbody>
          </table>
        </div>
