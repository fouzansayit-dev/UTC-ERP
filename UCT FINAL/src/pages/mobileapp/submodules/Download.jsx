...
export default function Download() {
  const [form, setForm] = useState({ course:'ALL', branchName:'', batch:'', date:today(), title:'', upload:null });
  ...
  const handleSubmit=()=>{
    ...
    setForm({course:'ALL',branchName:'',batch:'',date:today(),title:'',upload:null});
  };
  ...
  return (
    <div className="hr-form">
      ...
      <div style={{marginTop:28}}>
        <div style={{background:'#1e293b',color:'#fff',padding:'14px 20px',borderRadius:'10px 10px 0 0',fontWeight:700,fontSize:15}}>Download List</div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 0'}}>
          <div style={{display:'flex',gap:8}}>{['Copy','CSV','Print'].map(b=><button key={b} className="tbl-btn view" style={{background:'#f1f5f9',color:'#374151'}} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'Download'); else handlePrint('Download'); }}>{b}</button>)}</div>
        ...
        <div className="table-wrap" style={{borderRadius:'0 0 10px 10px'}}>
          <table className="hr-table">
            <thead><tr>{['S.No','Edit','Delete','Course','Branch Name','Batch','Date','Syllabus','Upload'].map(h=><th key={h}>{h}</th>)}</tr></thead>
            <tbody>
              {filtered.length===0?<tr><td colSpan={9} className="empty-table-msg">No data available in table</td></tr>
                :filtered.map((r,i)=><tr key={r.id}><td>{i+1}</td><td><button className="tbl-btn edit" onClick={()=>handleEdit(r)}>Edit</button></td><td><button className="tbl-btn del" onClick={()=>handleDelete(r.id)}>🗑</button></td><td>{r.course}</td><td>{r.branchName}</td><td>{r.batch}</td><td>{r.date}</td><td>{r.title}</td><td>{r.upload||'—'}</td></tr>)}
            </tbody>
          </table>
        </div>
