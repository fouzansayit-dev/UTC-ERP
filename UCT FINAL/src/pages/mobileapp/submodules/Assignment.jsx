...
export default function Assignment() {
  const [form, setForm] = useState({ course: '', branchName: '', batch: '', date: today(), subject: 'NA', assignment: '', attachment: null });
  ...
  const handleSubmit = () => {
    ...
    setForm({ course: '', branchName: '', batch: '', date: today(), subject: 'NA', assignment: '', attachment: null });
  };
  const handleEdit = row => { setEditId(row.id); setForm({ ...row }); };
  const handleDelete = id => setRows(p => p.filter(r => r.id !== id));
  const filtered = rows.filter(r => (r.assignment || '').toLowerCase().includes(search.toLowerCase()));
  return (
    <div className="hr-form">
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '24px 28px', boxShadow: '0 2px 8px rgba(67,97,238,0.06)' }}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          <div className="form-field"><label className="form-label">Assignment</label><textarea className="form-input form-textarea" rows={3} value={form.assignment} onChange={e => set('assignment', e.target.value)} /></div>
          <div className="form-field"><label className="form-label">Attachment</label><input className="form-input" type="file" onChange={e => set('attachment', e.target.files[0]?.name || null)} /></div>
        </div>
      ...
      <div style={{ marginTop: 28 }}>
        <div style={{ background: '#1e293b', color: '#fff', padding: '14px 20px', borderRadius: '10px 10px 0 0', fontWeight: 700, fontSize: 15 }}>Assignment List</div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0' }}>
          <div style={{ display: 'flex', gap: 8 }}>{['Copy','CSV','Print'].map(b=><button key={b} className="tbl-btn view" style={{ background:'#f1f5f9',color:'#374151' }} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'Assignment'); else handlePrint('Assignment'); }}>{b}</button>)}</div>
          ...
          <table className="hr-table">
            ...
            <tbody>
              {filtered.length === 0 ? <tr><td colSpan={10} className="empty-table-msg">No data available in table</td></tr>
                : filtered.map((r,i)=><tr key={r.id}><td>{i+1}</td><td><button className="tbl-btn edit" onClick={()=>handleEdit(r)}>Edit</button></td><td><button className="tbl-btn del" onClick={()=>handleDelete(r.id)}>🗑</button></td><td>{r.date}</td><td>{r.course}</td><td>{r.branchName}</td><td>{r.batch}</td><td>{r.subject}</td><td>{r.assignment}</td><td>{r.attachment||'—'}</td></tr>)}
            </tbody>
          </table>
        </div>
