...
export default function ViewInWard({ onBack }) {
  const [rows] = useState([]);
  ...
  return (
    <div className="hr-form">
      <div className="section-title">View In Ward</div>
      ...
      <div className="table-wrap">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['Copy', 'CSV', 'Print'].map(b => (
              <button key={b} className="tbl-btn view" style={{ background: '#f1f5f9', color: '#374151' }}
                onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'ViewInWard'); else handlePrint('View In Ward'); }}>{b}</button>
            ))}
          </div>
        ...
        <table className="hr-table">
          <thead>
            <tr>
              {['SNo.', 'Date', 'Letter No', 'From', 'Subject', 'Disposal Date', 'Reference No', 'Edit', 'Attachment'].map(h => (
                <th key={h}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={9} style={{ textAlign: 'center', color: '#888', padding: 20 }}>No data available in table</td></tr>
            ) : filtered.map((r, i) => (
              <tr key={r.id}>
                <td>{i + 1}</td>
                <td>{r.date}</td>
