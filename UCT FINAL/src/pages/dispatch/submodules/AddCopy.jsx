...
export default function AddCopy({ onBack }) {
  const [form, setForm] = useState({ copy: '', emailId: '' });
  ...
  const handleSubmit = () => {
    ...
    setRows(p => [...p, { id: Date.now(), copy: form.copy, emailId: form.emailId }]);
    setForm({ copy: '', emailId: '' });
  };
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Add Copy</div>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: '24px 28px', marginBottom: 24 }}>
        <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          <div className="form-field">
            ...
            <input className="form-input" value={form.copy} onChange={e => set('copy', e.target.value)} />
          </div>
          <div className="form-field">
            <label className="form-label">Email Id</label>
            <input className="form-input" type="email" value={form.emailId} onChange={e => set('emailId', e.target.value)} />
          </div>
        </div>
      ...
      <div className="table-wrap">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['Copy', 'CSV', 'Print'].map(b => (
              <button key={b} className="tbl-btn view" style={{ background: '#f1f5f9', color: '#374151' }}
                onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'AddCopy'); else handlePrint('Add Copy'); }}>{b}</button>
            ))}
          </div>
        ...
        <table className="hr-table">
          <thead>
            <tr>
              {['SNo.', 'Copy', 'Email Id'].map(h => <th key={h}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
