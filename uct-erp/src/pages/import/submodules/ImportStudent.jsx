...
const HEADERS = ['AdmNo','Name','FatherName','MotherName','Course','Batch','DOB','Gender','Mobile','Email','Address','Category','RollNo'];

function handleTableAction(action) {
  var tbl = document.querySelector('.hr-table'); if (!tbl) return;
  if (action === 'Print') {
    var w = window.open('', '_blank');
    w.document.write('<html><head><title>Import Student</title><style>body{font-family:Arial,sans-serif;padding:16px;font-size:12px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:5px 8px}th{background:#1e3a5f;color:#fff}tr:nth-child(even){background:#f5f5f5}</style></head><body><h2>Import Student Preview</h2>' + tbl.outerHTML + '</body></html>');
    ...
    var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'student_import.csv';
...
function downloadTemplate() {
  var csv = [HEADERS.join(','), 'ADM001,John Silva,Father Name,Mother Name,MBBS,2024,2005-01-15,Male,7771110001,john@uct.edu,Dili,General,R001'].join('\n');
...
export default function ImportStudent() {
  const [form, setForm] = useState({ session:'', course:'', batch:'' });
  ...
  function handleSubmit(e) {
    ...
    alert('Imported ' + preview.length + ' student record(s) from "' + file.name + '".');
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Import Student</div>
      ...
      <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:8, padding:'12px 16px', marginBottom:18, fontSize:13, color:'#1e40af' }}>
        ...
        <ul style={{ margin:'6px 0 0', paddingLeft:20, lineHeight:1.9 }}>
          <li>Download the template CSV, fill student data (one row per student)</li>
          <li>Required: AdmNo, Name, Course, Batch &nbsp;|&nbsp; Supported: <b>.csv, .xlsx</b></li>
        </ul>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-grid">
          ...
          <div className="form-field">
            ...
            <select className="form-input" value={form.batch} onChange={function(e) { set('batch', e.target.value); }}>
        ...
        <div className="form-submit-row">
          <button className="submit-btn" type="submit">Import Students</button>
        </div>
      </form>
