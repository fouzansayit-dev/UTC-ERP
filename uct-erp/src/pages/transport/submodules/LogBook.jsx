...
export default function LogBook() {
  ...
  const handleSubmit = () => {
    ...
    setForm({ vehicle: '', date: today(), startKm: '0', endKm: '0', runningKm: '0', diesel: '', remark: '' });
  };
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Add Log Book</div>
      ...
      <div className="form-grid" style={{ maxWidth: 600 }}>
        ...
        <div className="form-field">
          ...
          <input className="form-input" type="number" min="0" value={form.startKm} onChange={e => set('startKm', e.target.value)} />
        </div>
        <div className="form-field">
        ...
        <div className="form-field">
          ...
          <input className="form-input" type="number" min="0" step="0.1" placeholder="Litres" value={form.diesel} onChange={e => set('diesel', e.target.value)} />
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 28 }}>
        <div style={{ display: 'flex', gap: 8 }}>
          {['Copy', 'CSV', 'Print'].map(b => (
            <button key={b} className="tbl-btn view" style={{ background: '#f1f5f9', color: '#374151' }} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'LogBook'); else handlePrint('Log Book'); }}>{b}</button>
          ))}
        </div>
      ...
      <div className="table-wrap">
        <table className="hr-table">
          <thead>
            <tr>
              {['SNo.','Vehicle','Date','Start Kilometre','End Kilometre','Running Kilometer','Diesel','Remark','Delete'].map(h => (
                <th key={h}>{h}</th>
              ))}
            </tr>
          </thead>
