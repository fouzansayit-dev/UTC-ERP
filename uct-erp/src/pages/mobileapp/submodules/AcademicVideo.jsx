...
export default function AcademicVideo() {
  const [form, setForm] = useState({ course:'', branchName:'', batch:'', date:today(), subject:'NA', chapter:'', chapterTopics:'', youtubeLink:'' });
  const [rows, setRows] = useState([]);
  const [editId, setEditId] = useState(null);
  const [search, setSearch] = useState('');
  const set=(k,v)=>setForm(p=>({...p,[k]:v}));
  const handleSubmit=()=>{
    if(!form.youtubeLink) return;
    if(editId!==null){setRows(p=>p.map(r=>r.id===editId?{...r,...form}:r));setEditId(null);}
    else setRows(p=>[...p,{id:Date.now(),...form}]);
    setForm({course:'',branchName:'',batch:'',date:today(),subject:'NA',chapter:'',chapterTopics:'',youtubeLink:''});
  };
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Academic Video</div>
      <div style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:10,padding:'24px 28px',boxShadow:'0 2px 8px rgba(67,97,238,0.06)'}}>
        <div className="form-grid" style={{gridTemplateColumns:'1fr 1fr'}}>
          ...
          <div className="form-field"><label className="form-label">Chapter / Topics</label><textarea className="form-input form-textarea" rows={2} value={form.chapterTopics} onChange={e=>set('chapterTopics',e.target.value)} /></div>
          <div className="form-field" style={{gridColumn:'1 / -1'}}><label className="form-label">Youtube Link</label><textarea className="form-input form-textarea" rows={2} value={form.youtubeLink} onChange={e=>set('youtubeLink',e.target.value)} placeholder="https://youtube.com/..." /></div>
        </div>
      ...
      <div style={{marginTop:28}}>
        <div style={{background:'#1e293b',color:'#fff',padding:'14px 20px',borderRadius:'10px 10px 0 0',fontWeight:700,fontSize:15}}>Academic Video List</div>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 0'}}>
          <div style={{display:'flex',gap:8}}>{['Copy','CSV','Print'].map(b=><button key={b} className="tbl-btn view" style={{background:'#f1f5f9',color:'#374151'}} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'AcademicVideo'); else handlePrint('Academic Video'); }}>{b}</button>)}</div>
        ...
        <div className="table-wrap" style={{borderRadius:'0 0 10px 10px'}}>
          <table className="hr-table">
            ...
            <tbody>
              {filtered.length===0?<tr><td colSpan={11} className="empty-table-msg">No data available in table</td></tr>
                :filtered.map((r,i)=><tr key={r.id}><td>{i+1}</td><td><button className="tbl-btn edit" onClick={()=>handleEdit(r)}>Edit</button></td><td><button className="tbl-btn del" onClick={()=>handleDelete(r.id)}>🗑</button></td><td>{r.date}</td><td>{r.course}</td><td>{r.branchName}</td><td>{r.batch}</td><td>{r.subject}</td><td>{r.chapter}</td><td>{r.chapterTopics}</td><td style={{maxWidth:120,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.youtubeLink}</td></tr>)}
            </tbody>
          </table>
        </div>
