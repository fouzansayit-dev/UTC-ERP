...
export default function AcademicNotesDetails() {
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Academic Notes Details</div>
      ...
      {submitted&&(
        <div style={{marginTop:28}}>
          <div style={{background:'#1e293b',color:'#fff',padding:'14px 20px',borderRadius:'10px 10px 0 0',fontWeight:700,fontSize:15}}>Academic Notes Details ({fmt(form.startDate)} &ndash; {fmt(form.endDate)})</div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 0'}}>
            <div style={{display:'flex',gap:8}}>{['Copy','CSV','Print'].map(b=><button key={b} className="tbl-btn view" style={{background:'#f1f5f9',color:'#374151'}} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'AcademicNotesDetails'); else handlePrint('Academic Notes Details'); }}>{b}</button>)}</div>
          ...
          <div className="table-wrap" style={{borderRadius:'0 0 10px 10px'}}>
            <table className="hr-table">
              ...
              <tbody>{filtered.length===0?<tr><td colSpan={12} className="empty-table-msg">No data available in table</td></tr>:null}</tbody>
            </table>
          </div>
