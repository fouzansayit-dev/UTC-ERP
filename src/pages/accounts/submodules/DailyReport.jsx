...
export default function DailyReport() {
  ...
  const calculateReport = (data) => {
    ...
    // Compute Opening Balance (prior to start date)
    let opBal = 0;
    sorted.forEach(r => {
    ...
    // Compute running balance for rows
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Daily Report</div>
      ...
      {shown && (
        <div className="table-wrap">
          <TableToolbar title="UCT ERP Daily Report" />
          <table className="hr-table">
            ...
            <tbody>
              ...
              {rows.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign:'center', color:'#9ca3af', padding:20 }}>No transactions in this date range</td></tr>
              ) : rows.map((r, i) => (
                <tr key={r.id}>
                  <td>{i+1}</td>
                  <td>{r.date}</td>
                  <td>{r.party || r.head || 'General Cash Entry'} {r.remarks ? `(${r.remarks})` : ''}</td>
