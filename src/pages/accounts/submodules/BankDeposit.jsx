...
export default function BankDeposit() {
  ...
  const handleSubmit = () => {
    ...
    fetch('/api/generic/accounts/deposits', {
      ...
      .catch(err => alert('Failed to save deposit: ' + err.message));
  };
  ...
  return (
    <div className="hr-form">
      ...
      <div className="table-wrap">
        <TableToolbar title="UCT ERP Bank Deposits Ledger" />
        <table className="hr-table">
          ...
          <tbody>
            {rows.length === 0 ? <EmptyRow cols={9} /> : rows.map((r, i) => (
              <tr key={r.id}><td>{i+1}</td><td>{r.bank}</td><td>{r.date}</td><td>₹{parseFloat(r.amount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td><td>{r.status}</td><td>{r.payMode}</td><td>{r.chequeNo || '-'}</td><td>{r.remarks || '-'}</td>
