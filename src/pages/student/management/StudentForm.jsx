...
export default function StudentForm({ initialData = {}, onSubmit, submitLabel = 'Submit', readOnly = false }) {
  ...
  useEffect(() => {
    fetch('/api/consultants')
      .then(res => res.json())
      .then(data => setDbConsultants(data))
      .catch(err => console.error(err));
    fetch('/api/agents')
      .then(res => res.json())
      .then(data => setDbAgents(data))
      .catch(err => console.error(err));
  }, []);
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="stu-form-grid">
        ...
        <div className="stu-field">
          ...
          <select value={form.consultant || ''} onChange={set('consultant')} disabled={readOnly}>
            <option value="">-- Select --</option>
            {dbConsultants.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
          </select>
        </div>
          ...
          <div className="stu-form-grid" style={{ marginBottom: 16 }}>
            ...
            <div className="stu-field">
              ...
              <select value={form.agentName || ''} onChange={(e) => {
                const selected = dbAgents.find(a => a.firm_name === e.target.value);
                setForm(p => ({ ...p, agentName: e.target.value, agentId: selected ? selected.agent_code : '' }));
              }} disabled={readOnly}>
                <option value="">-- Select Agent --</option>
                {dbAgents.map(a => <option key={a.id} value={a.firm_name}>{a.firm_name}</option>)}
              </select>
            </div>
            <div className="stu-field">
              <label>Agent ID</label>
              <input value={form.agentId || ''} readOnly placeholder="Auto-populated" />
            </div>
          </div>
