...
export default function GatePassDetailsE() {
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Gate Pass Details (Employee)</div>
      ...
      {submitted && (
        <div style={{ marginTop: 28 }}>
          <div style={{
            ...
            Gate Pass Details (E) — ({fmt(form.startDate)} &ndash; {fmt(form.endDate)})
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0' }}>
            <div style={{ display: 'flex', gap: 8 }}>
              {['Copy', 'CSV', 'Print'].map(b => (
                <button key={b} className="tbl-btn view" style={{ background: '#f1f5f9', color: '#374151' }} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'GatePassDetailsE'); else handlePrint('Gate Pass Details E'); }}>{b}</button>
              ))}
            </div>
          ...
          <div className="table-wrap" style={{ borderRadius: '0 0 10px 10px' }}>
            <table className="hr-table">
              <thead>
                <tr>
                  {['S.No', 'Date', 'Name', 'Validate Date/Time', 'Issue Date/Time', 'Reason', 'Permitted By', 'Print'].map(h => (
                    <th key={h}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={i}>
                    ...
                    <td>{r.reason}</td>
                    <td>{r.permittedBy}</td>
                    <td><button className="tbl-btn view" onClick={() => handlePrint('Gate Pass Details E')}>Print</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
