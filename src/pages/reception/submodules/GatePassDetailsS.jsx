...
export default function GatePassDetailsS() {
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Gate Pass Details (Student)</div>
      ...
      {submitted && (
        <div style={{ marginTop: 28 }}>
          <div style={{
            ...
            Gate Pass Details (S) — ({fmt(form.startDate)} &ndash; {fmt(form.endDate)})
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0' }}>
            <div style={{ display: 'flex', gap: 8 }}>
              {['Copy', 'CSV', 'Print'].map(b => (
                <button key={b} className="tbl-btn view" style={{ background: '#f1f5f9', color: '#374151' }} onClick={e => { if(b==='Copy') handleCopy(e.currentTarget); else if(b==='CSV') handleCSV(e.currentTarget, 'GatePassDetailsS'); else handlePrint('Gate Pass Details S'); }}>{b}</button>
              ))}
            </div>
          ...
          <div className="table-wrap" style={{ borderRadius: '0 0 10px 10px' }}>
            <table className="hr-table">
              <thead>
                <tr>
                  {['S.No', 'Print', 'Type', 'Name', 'Validate Date/Time', 'Issue Date/Time', 'Accompanied By', 'Relation', 'Reason', 'Permitted By'].map(h => (
                    <th key={h}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={i}>
                    <td>{i + 1}</td>
                    <td><button className="tbl-btn view" onClick={() => handlePrint('Gate Pass Details S')}>Print</button></td>
                    <td>{r.type}</td>
                    <td>{r.name}</td>
