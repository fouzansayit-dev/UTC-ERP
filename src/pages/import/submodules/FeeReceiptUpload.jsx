...
const HEADERS = ['AdmNo','StudentName','ReceiptNo','ReceiptDate','FeeHead','Amount','PaymentMode','Remarks'];

function handleTableAction(action) {
  var tbl = document.querySelector('.hr-table'); if (!tbl) return;
  if (action === 'Print') {
    var w = window.open('', '_blank');
    w.document.write('<html><head><title>Fee Receipt Upload</title><style>body{font-family:Arial,sans-serif;padding:16px;font-size:12px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:5px 8px}th{background:#1e3a5f;color:#fff}tr:nth-child(even){background:#f5f5f5}</style></head><body><h2>Fee Receipt Upload Preview</h2>' + tbl.outerHTML + '</body></html>');
    ...
    var a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'fee_receipt_upload.csv';
...
function downloadTemplate() {
  var csv = [HEADERS.join(','), 'ADM001,John Silva,RCP001,2024-06-01,Tuition Fee,15000,Cash,'].join('\n');
  var a = document.createElement('a');
  a.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }));
  a.download = 'fee_receipt_upload_template.csv';
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
}

export default function FeeReceiptUpload() {
  ...
  function handleSubmit(e) {
    ...
    alert('Uploaded ' + preview.length + ' fee receipt record(s) from "' + file.name + '".');
  ...
  return (
    <div className="hr-form">
      <div className="section-title">Fee Receipt Upload</div>
      <div style={{ background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:8, padding:'12px 16px', marginBottom:18, fontSize:13, color:'#1e40af' }}>
        ...
        <ul style={{ margin:'6px 0 0', paddingLeft:20, lineHeight:1.9 }}>
          <li>Download template, fill fee receipt data. Required: AdmNo, ReceiptNo, Amount</li>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-submit-row"><button className="submit-btn" type="submit">Upload Fee Receipts</button></div>
      </form>
      {preview.length > 0 && (
        <div>
